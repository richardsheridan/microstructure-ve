...
For every kept matrix cell x {elastic, viscoelastic} we build the Simulation (which runs
``validate_constraints`` at Model construction -- a passing build is itself the over-
constraint check), emit the ``.inp`` in memory, and assert its structure: the drive
``*Boundary, type=displacement`` on the right corner/face and dof, the periodic vs
standard ``*Equation`` presence, the traction-specific fixed DOFs, and the step type.

These are parsed assertions, not per-cell byte goldens -- the committed ``synthetic.inp``
remains the formatting-stability guard. Numeric ABAQUS oracles come in a later pass.
"""
import unittest
...
class MatrixAbaqusStructureTests(unittest.TestCase):
    def test_emitted_inp_structure(self):
        for cell in matrix_cells():
            for test_type in REAL_TEST_TYPES:
                with self.subTest(test_type=test_type, **cell):
                    ...
                    # step type / perturbation flag
                    self.assertEqual(parsed["step_type"], exp["step_type"])
                    self.assertEqual(parsed["perturbation"], exp["perturbation"])
