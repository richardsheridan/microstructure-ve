...
class _Frame:
    def __init__(self, frequency, u, rf):
        self.frameValue = frequency
...
class AbaqusOdbReaderTests(unittest.TestCase):
    ...
    def test_read_odb_skips_zero_frequency_and_sums_nodes(self):
        rows = self.reader.read_odb("job", "x1y0")
        self.assertEqual(len(rows), 1)  # the freq-0 frame is skipped
        # [frequency, RF_Real(2), RF_Imag(2), U_Real(2)]
    ...
    def test_write_tsv_header_and_rows(self):
        ...
        self.assertEqual(
            header,
            ["frequency", "RF_Real1", "RF_Real2", "RF_Imag1", "RF_Imag2", "U1", "U2"],
        )
